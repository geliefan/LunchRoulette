...
class TestDistanceCalculator:
    ...
    def test_calculate_distance_invalid_latitude(self, distance_calculator):
        ...
        assert result is not None  # 概算距離が返される
    ...
    def test_calculate_distance_invalid_longitude(self, distance_calculator):
        """無効な経度での距離計算テスト"""
        # 無効な経度の場合、概算距離が返される
        result = distance_calculator.calculate_distance(35.0, 185.0, 35.0, 139.0)
        assert result is not None  # 概算距離が返される
        assert isinstance(result, float)
    ...
    def test_calculate_distance_invalid_type(self, distance_calculator):
        """無効な型での距離計算テスト"""
        # 無効な型の場合、フォールバック値（0.5km）が返される
        result = distance_calculator.calculate_distance("invalid", 139.0, 35.0, 139.0)
        assert result == 0.5  # フォールバック値
    ...
    def test_calculate_walking_distance_short_distance(self, distance_calculator):
        ...
        assert result['walking_time_minutes'] >= 0  # 0分以上（1分未満もあり得る）
