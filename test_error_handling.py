...
class TestErrorHandling:
    ...
    def test_cache_service_serialization_error_handling(self, cache_service):
        ...
        # シリアライズできないオブジェクト
        unserializable_data = lambda x: x  # 関数オブジェクト
    ...
    def test_location_service_rate_limit_handling(self, mock_get, cache_service):
        ...
        # 事前にキャッシュデータを設定
        cache_key = cache_service.generate_cache_key('location', ip='192.168.1.1')
        old_data = {
            'latitude': 35.6762,
            'longitude': 139.6503,
            'city': '千代田区',
    ...
    def test_graceful_degradation_scenario(self, cache_service):
        ...
        assert restaurant_result is not None  # 空リストでも None ではない
